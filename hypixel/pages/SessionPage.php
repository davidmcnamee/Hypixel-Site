...
class SessionPage extends Page {
    ...
    //private $otherPlayerResults;
    
    public function __construct($sessionResult,$playerResult) {
        $this->playerResult = $playerResult;
        $this->sessionResult = $sessionResult;
        $this->otherPlayerResults = array();
    }
    
    public static function fixExtraValues($extraValues) {
        foreach($extraValues as $key => $value) {
            if(Draw::$StatRequests[$value]===null) {
                unset($extraValues[$key]);
            }
        }
        if(array_search(0,$extraValues)===false) {
            array_unshift($extraValues, 0);
        }
        if($extraValues[0]!==0) {
            unset($extraValues[array_search(0,$extraValues)]);
            array_unshift($extraValues, 0);
        }
        return $extraValues;
    }
...
<html>
    <head>
        <title>
            <?php echo $this->playerResult->getUnformattedName(); ?>
        </title>
    ...
    <?php
        ...
        $extraValues = self::fixExtraValues($extraValues);
        $table = array();
        foreach($extraValues as $id) {
            $table[0][Draw::$StatRequests[$id][0]] = Draw::$StatRequests[$id][1];
        }
        
        foreach($this->sessionResult->getUUIDList() as $uuid) {
...
<body style="background-color:F8F8F8;" onload="loadStats()">
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark">
            ...
            <a class="navbar-brand" href="">
            <?php
                if(strpos($_SERVER['HTTP_HOST'], 'thelagg') !== false) {
                    echo "Lagg";
                } else {
                    echo "David M";
                }
...
<div class="container">
    ...
    <h2 style="padding-top: 20px; padding-bottom: 20px"><?php echo $this->sessionResult->getGameType(); ?></h2>  
        <div class="dropdown">
    ...
    <tbody>
      ...
      foreach($table as $row) {
        ...
        foreach($row as $index => $value) {
            $justify = 'left';
            $bold = '';
            if(strpos($tableheader[$index], ' b') !== false) {
                $bold = ' font-weight: bold;';
            }
            if($tableheader[$index]==='num') {
                $justify = 'right';
            }
